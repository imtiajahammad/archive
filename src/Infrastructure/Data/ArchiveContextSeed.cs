...
{
    public static async Task SeedAsync(ArchiveContext archiveContext,
        ILogger logger,
        int retry = 0)
    {
        ...
        try
        {
            if (archiveContext.Database.IsSqlServer())
            {
                archiveContext.Database.Migrate();
            }

            /*if (!await archiveContext.CatalogBrands.AnyAsync())
            {
                await archiveContext.CatalogBrands.AddRangeAsync(
                    GetPreconfiguredCatalogBrands());

                await archiveContext.SaveChangesAsync();
            }

            if (!await archiveContext.CatalogTypes.AnyAsync())
            {
                await archiveContext.CatalogTypes.AddRangeAsync(
                    GetPreconfiguredCatalogTypes());

                await archiveContext.SaveChangesAsync();
            }

            if (!await archiveContext.CatalogItems.AnyAsync())
            {
                await archiveContext.CatalogItems.AddRangeAsync(
        ...
        catch (Exception ex)
        {
            ...
            await SeedAsync(archiveContext, logger, retryForAvailability);
            throw;
        }
    }
/*
